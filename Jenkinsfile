        ...
        stage ('Deploy') {
            steps {
              withCredentials([file(credentialsId: 'angular-react-deployment-server', variable: 'deployment_server')]) {
                   sh 'scp -v -i ${deployment_server} ankushApp.zip ubuntu@18.188.202.13:/home/ubuntu'
                   sh 'ssh -v -i ${deployment_server} ubuntu@18.188.202.13 "cd /home/ubuntu;unzip -o ankushApp.zip -d angularApp;cd angularApp;pm2 restart angularApp"'
               }
            }
        }
